...
# If $directory is omitted, creates `LeanBourbaki.lean`.

cd "$( dirname "${BASH_SOURCE[0]}" )"/../LeanBourbaki
if [[ $# = 1 ]]; then
...
# prepend `import «`
find "$dir" -name \*.lean -not -name LeanBourbaki.lean \
  | sed 's,^\./,,;s,^\.\./[^/]*,,;s,/,.,g;s,\.lean$,,;s,^,import LeanBourbaki.,' \
  | sort >"$dir"/../LeanBourbaki.lean
